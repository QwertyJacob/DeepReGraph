...
def get_initial_state(some_clustering):
  ...
  for k in cluster_labels:
      ...
      gene_cluster_sizes[k] = elems_per_class[0] / max_conventional_cluster_shape
      ccre_cluster_sizes[k] = elems_per_class[1] / max_conventional_cluster_shape
...
for episode in range(training_episodes):
    ...
    # state variables:
    cluster_labels = np.unique(init_clustering)
    ...
    for step in tqdm(range(element_count)):
        ...
        linear_state_suffix = torch.Tensor([step / element_count, gene_completeness, ccre_completeness])
        ...
        previous_gene_dispersion = np.nan_to_num(gene_mean_dispersions)[decoded_action].copy()
        previous_ccre_dispersion = np.nan_to_num(ccre_mean_dispersions)[decoded_action].copy()

        if step % HETEROGENEITY_SCORE_UPDATE == 0:
            previous_heterogeneity = np.nan_to_num(cluster_heterogeneities)[decoded_action].copy()
        if step % DISTANCE_SCORE_UPDATE_INTERVAL == 0:
            previous_distance_score = np.nan_to_num(distance_scores)[decoded_action].copy()
