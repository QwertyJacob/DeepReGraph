...
class AdaGAE(torch.nn.Module):
    ...
    def __init__(self, X, layers=None,
                 init_sparsity=150, update=True, links=None, device=None,
                 pre_trained=False,
        ...
        self.X = X
        self.init_sparsity = init_sparsity + 1
    ...
    def update_graph(self, epoch, genetic_balance_factor):
        print('updating graph Laplacian with neighbors: ', self.init_sparsity)
        tensorboard.add_scalar(NUM_NEIGHBORS_LABEL, self.init_sparsity, epoch * max_iter)
    ...
    def run(self):
        tensorboard.add_scalar(NUM_NEIGHBORS_LABEL, self.init_sparsity, 0)
        if self.pre_trained:
        ...
        for epoch in tqdm(range(max_epoch)):
            ...
            if (not bounded_sparsity) or (self.init_sparsity < self.max_neighbors):
                ...
                self.init_sparsity += sparsity_increment
            else:
                if self.update:
                    self.init_sparsity = int(self.max_neighbors)
                    break
                recons = None
                weights = weights.cpu()
                raw_weights = raw_weights.cpu()
